...
const QuoteInputForm: React.FC<QuoteInputFormProps> = ({ distributor, initialPrices, onPriceChange, onSubmit, onCancel, disabled = false }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className={disabled ? 'opacity-50 pointer-events-none' : ''}>
          ...
          <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-slate-800">
            ...
            <button
            type="submit"
            disabled={disabled || isSubmitting || Object.values(initialPrices).every(p => !p)}
            ...
            >
            {isSubmitting ? 'Enviando...' : disabled ? 'Dia Sem Mercado' : 'Enviar Cotação'}
            </button>
          </div>
        </form>
    );
