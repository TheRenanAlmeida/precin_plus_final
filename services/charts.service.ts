...
export const fetchChartDataForDashboard = async (
    ...
    try {
        ...
        if (distinctDatesError) {
            if (distinctDatesError.message?.includes('aborted')) return { rawChartData: [], userHistoryChartData: [], error: null };
        ...
        if (err.name === 'AbortError' || signal?.aborted) return { rawChartData: [], userHistoryChartData: [], error: null };
        return { rawChartData: null, userHistoryChartData: null, error: `Falha de rede ao carregar gráficos.` };
    }
};
